...
import (
    ...
    "github.com/ardanull/distributed-commerce-core/internal/services/inventory"
)
...
func main() {
    ...
    svc := inventory.NewService(db, inbox.New(db), bus, logger)
    bg := func(ctx context.Context) error {
        _, err := bus.SubscribeDurable("inventory.reserve.*", "inventory-reserve", svc.HandleReserve)
        return err
    }
