...
import (
    ...
    "github.com/ardanull/distributed-commerce-core/internal/services/payment"
)
...
func main() {
    ...
    svc := payment.NewService(db, inbox.New(db), bus, logger)
    bg := func(ctx context.Context) error {
        _, err := bus.SubscribeDurable("payment.authorize.*", "payment-authorize", svc.HandleAuthorize)
        if err != nil { return err }
        _, err = bus.SubscribeDurable("payment.refund.*", "payment-refund", svc.HandleRefund)
        return err
    }
